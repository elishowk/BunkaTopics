...
import { SearchCustomOptions } from '../types'
...
/**
 * Component used in BourdieuMap
 */
export default function AddContinuumDimensionsListItem(
  props: AddContinuumDimensionsListItemProps
) {
  ...
  const [autocompleted2, setAutocompleted2] =
    useState<DimensionAutocomplete | null>(null)
  ...
  useEffect(() => {
    if (autocompleted2) {
      const dim = dimensionsSuggestions.find(
        (ds) => ds.type === autocompleted2.type
      )
      if (dim != null) setCustomKeywords2(dim.words ?? [])
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autocompleted2])
              ...
              <Box flex='1' display='flex' flexDirection='column'>
                <FormControl>
                  ...
                  <Autocomplete
                    ...
                    freeSolo
                    renderInput={(params) => (
                      <TextField name='dimensionId2' {...params} />
                    )}
                  />
                </FormControl>
