...
import type { Document, ResultsMetadata, ResultsSortOrder, SearchResponse, Topic, ViewDimensions } from '../../types'
...
export const SearchResultsWithRef = forwardRef<SearchResultsHandler, SearchResultsProps>(function SearchResults({
  documents,
  view,
  dimensions,
  topicSelected,
      ...
      view === Views.BOURDIEU_MAP
        ? dimensions.continuum
          ...
          .map((dim) => [
            {
              id: dim.type,
              title: dim.idLeft != null && dim.idLeft.length > 0 ? dim.idLeft[0].toUpperCase() + dim.idLeft.slice(1) : ''
            },
            {
              id: dim.type,
              title: dim.idRight != null && dim.idRight.length > 0 ? dim.idRight[0].toUpperCase() + dim.idRight.slice(1) : '',
              asc: true
            }
          ])
  ...
  const filteredDocuments = documents
    // apply search
    .filter((rd) => searchFilter === "" || rd.text.toLowerCase().indexOf(searchFilter.toLowerCase()) >= 0)
      ...
      <Box padding="0.75em" width="100%" display="flex" flexDirection="row">
        <Box flexGrow={1}>
          <TextField
            ...
            size='small'
            InputProps={{
              endAdornment: searchFilter == null || searchFilter === "" ? (
        ...
        <Box flexShrink={1}>
          ...
          >
            {sortOptions.map((sortOption) => (
              <MenuItem
                key={`${sortOption.id}-${sortOption.asc === true ? 'asc' : 'desc'}`}
                selected={sortOrder === sortOption.id && sortOption.asc === sortDirectionAsc}
                onClick={() => {
                  CustomEvent.trackEvent('results', 'sort', `${sortOption.id}-${sortOption.asc === true ? 'asc' : 'desc'}`)
          ...
          {filteredDocuments
            .map((document, i, a) => {
              ...
              return (
                ...
                >
                  <Box borderBottom={i < a.length - 1 ? "1px solid lightgrey" : "none"} width="100%">
                    <Box display="flex" flexDirection="column">
                      <Box display="flex">
                        {document.author.picture != null && document.author.picture !== "" && (
                          <Box
                            padding="0.4rem"
                            lineHeight="0"
                          >
                        ...
                        <Box paddingY="0.2rem">
                          ...
                          <Box sx={({ palette }) => ({
                            ...
                            <a href={`${document.author.url}`} target="_blank" rel="nofollow noreferrer">
                    ...
                    <Typography padding="0.4rem" fontSize="0.8rem">{document.text}</Typography>
                    {(document.created_at_timestamp_sec ?? 0) > 0 && (
                      ...
                      >
                        <a href={`https://${document.id}`} target="_blank" rel="nofollow noreferrer">
                          {moment.unix(document.created_at_timestamp_sec ?? 0).format('ll')}
                        </a>
                      </Typography>
                    )}

                    <DocumentChips document={document} dimensions={dimensions.intensity} />
                  </Box>
                </ListItem>
              )
            })
