...
export const StyledMap = styled.div<{ theme: Theme }>(
  ({ theme }) => css`
  position: relative;
  width: 100%;
  height: 100%;
...
const BourdieuMapWithRef = forwardRef<ViewHandler, BourdieuMapProps>(
  function BourdieuMap(props, ref) {
    ...
    const [dimensionX, setDimensionX] = useState<string | DimensionType>(
      props.dimensionX ?? 'positive / negative'
    )
    const [dimensionY, setDimensionY] = useState<string | DimensionType>(
      props.dimensionY ?? 'national / international'
    )
    ...
    useLayoutEffect(() => {
      ...
      return () => {
        // clear map
        D3.select('#bourdieuMap').selectChildren().remove()
      }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [
      dimensionX,
    ...
    return (
      <Box
        ...
        sx={{
          backgroundColor:
            props.interpolation === 'terrain' ? 'white' : 'inherit'
        }}
      >
        <StyledMap
          id='bourdieuMap'
        ...
        <Box position='absolute' top='1em' left='1em' display='block'>
          <Card variant='outlined' sx={{ paddingY: '0.5em' }}>
            ...
            <List dense disablePadding>
              {[...(props.dimensions?.continuum ?? [])].map(
                ({ type, label }) => (
                  <ListItem
                    dense
                    disablePadding
