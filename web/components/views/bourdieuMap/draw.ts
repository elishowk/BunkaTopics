...
export default function draw(
  elementId: string,
  props: BourdieuMapProps & { screenSize: ScreenSize }
) {
  const {
    documents,
    screenSize,
  ...
  const dimensionX = dimensions.continuum.find(
    (d) =>
      (d.type as string).toLowerCase() ===
      (props.dimensionX as string).toLowerCase()
  )
  const dimensionXType = ((dimensionX?.type ?? '') as string).toLowerCase()
  const dimensionY = dimensions.continuum.find(
    (d) =>
      (d.type as string).toLowerCase() ===
      (props.dimensionY as string).toLowerCase()
  )
  ...
  const contoursGenerator = d3
    ...
    .x((d) =>
      x(
        d.dimensions?.find((dd) => dd.id.toLowerCase() === dimensionXType)
          ?.score ?? 0
      )
    )
    .y((d) =>
      y(
        d.dimensions?.find((dd) => dd.id.toLowerCase() === dimensionYType)
          ?.score ?? 0
      )
    )
    .size([width, height])
  ...
  var color = d3
    ...
    .domain(d3.extent(densityData.map((d) => d.value)) as [number, number])
  ...
  const documentsPoints = view
    ...
    .attr('cx', (doc) =>
      x(
        doc.dimensions?.find((d) => d.id.toLowerCase() === dimensionXType)
          ?.score ?? 0
      )
    )
    .attr('cy', (doc) =>
      y(
        doc.dimensions?.find((d) => d.id.toLowerCase() === dimensionYType)
          ?.score ?? 0
      )
    )
    .attr('r', 2)
    ...
    .on('mouseout', function (event, document) {
      d3.select<SVGCircleElement, Document>(event.target)
        ...
        .attr('opacity', (doc) =>
          _documentSelected != null && doc.id === _documentSelected.id ? 1 : 0.5
        )
    })
  ...
  if (color(0) !== 'none') {
    ...
    const colorscale = svg.insert('g').classed('colorscale', true)
    colorscale
      .insert('text')
  ...
  canvas
    ...
    .on('click', function (event, datum) {
      const { dimension } = d3
        .select<SVGGElement, { dimension: typeof dimensionX }>(
   ...
   * Events on Map
   */
  map.on('documenttargetted', function (event: DocumentSelectedEvent) {
