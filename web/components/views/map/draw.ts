...
export default function draw(
  elementId: string,
  props: MapProps & { screenSize: ScreenSize }
) {
  const {
    documents,
    topics,
   ...
   */
  const svg = map.append('svg').attr('width', width).attr('height', height)
   ...
   * Scales.
   */
  const xRangeComputed = d3.extent(
    documents?.map((d) => d.embedding_light[0]) ?? [0, 10]
  ) as [number, number]
  ...
  const yRangeComputed = d3.extent(
    documents?.map((d) => d.embedding_light[1]) ?? [0, 10]
  ) as [number, number]
  ...
  var x = d3.scaleLinear().domain(xRange).range([0, width])
  ...
  const zoom = d3
    ...
    .on('zoom', function ({ transform }) {
      canvas.attr(
        'transform',
        `translate(${transform.x ?? 0}, ${transform.y ?? 0}) scale(${
          transform.k ?? 1
        })`
      )
      positionLabels()
   ...
   * Contours generator.
   */
  const contoursGenerator = d3
    .contourDensity<Document>()
    .x((d) => x(d.embedding_light[0])) // x and y = column name in .csv input data
    .y((d) => y(d.embedding_light[1]))
    .weight((_d) => {
      if (
        dimension === DimensionType.NONE ||
        dimension === DimensionType.DENSITY
      )
        return 1
      const score =
        _d.dimensions?.find((dd) => dd.id.toLowerCase() === dimension)?.score ??
        0
      const threshold =
        dimension === 'popularity' &&
        process.env.NEXT_PUBLIC_POPULARITY_SCORE_THRESHOLD != null
  ...
  canvas
    ...
    .attr(
      'opacity',
      interpolation === 'terrain' && dimension !== DimensionType.NONE ? 0.25 : 1
    )
  ...
  topics?.forEach((topic) => {
    map
      ...
      .style('display', (topic) => {
        ...
        return (topic.parent_topic_id == null &&
          initialTransform.k < trigger) ||
      ...
      .on('mouseover', function (event) {
        ...
        d3.select(`#topic-${topic.id}-area`).attr('fill-opacity', 0.4)
      })
      .on('mouseout', function (event) {
        d3.select(`#topic-${topic.id}-area`).attr('fill-opacity', 0)
      ...
      .text(
        topic.explanation.specific_terms.filter((_, i) => i < 5).join(' | ')
      )
  })
  ...
  const convexHulls = canvas
    ...
    .style('display', (topic) => {
      return topic.parent_topic_id != null &&
        initialTransform.k > (triggerSubtopics ?? 2)
        ? 'unset'
        : topic.parent_topic_id == null
        ? 'unset'
        : 'none'
  ...
  convexHulls
    ...
    .attr('points', (t) => {
      return t.convex_hull.x_coordinates != null &&
        ...
        ? [...new Array<string>(t.convex_hull.x_coordinates.length)].map(
            (_, i) =>
              [
                // @ts-ignore
    ...
    .attr('fill', (t) => {
      return _topicSelected?.id == null || t.id !== _topicSelected.id
        ? areaPrimaryColor
        : 'red'
    })
    ...
    .on('mouseout', function (event, topic) {
      d3.select(this)
        .attr(
          'fill',
          _topicSelected?.id == null || topic.id !== _topicSelected.id
            ? areaPrimaryColor
            : 'red'
        )
        .attr(
          'fill-opacity',
          _topicSelected?.id == null || topic.id !== _topicSelected.id ? 0 : 0.4
        )
        .style('cursor', 'unset')
    })
  ...
  const documentsPoints = canvas
    ...
    .attr(
      'id',
      (doc) => `document-${doc.id.replaceAll('/', '-').replaceAll('.', '_')}`
    )
    .append('circle')
    ...
    .attr('fill', (doc) =>
      _documentSelected?.id != null && doc.id === _documentSelected?.id
        ? 'red'
        : 'blue'
    )
    .attr('opacity', (doc) =>
      _documentSelected?.id != null && doc.id === _documentSelected.id ? 1 : 0.5
    )
    .on('mouseover', function (event) {
    ...
    .on('mouseout', function (event, document) {
      d3.select<SVGCircleElement, Document>(event.target)
        .attr('r', 2)
        .attr('fill', (doc): string =>
          _documentSelected != null && doc?.id === _documentSelected.id
            ? 'red'
            : 'blue'
        )
        .attr('opacity', (doc) =>
   ...
   */
  if (color(0) !== 'none') {
    const colorscaleColor = (color as d3.ScaleSequential<string, never>).domain(
      [1, 0]
    )
   ...
   * Document selection.
   */
  map.on('topicselected', function (event) {
